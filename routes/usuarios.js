...
router.post("/usuarios/add", async (req, res, next) => {
  ...
  let schema = yup.object({
    ...
    username: yup
      ...
      .lowercase("O username deve ser minusculos"),
...
router.get("/usuarios/edit-user/:id", async (req, res, next) => {
  try {
    ...
    const { id } = req.params;
    const [usuarioEditar] = await database("usuarios").where("id", id);
...
router.delete("/usuarios/:id", async (req, res, next) => {
  try {
    const { id } = req.params;
    const result = await database("usuarios").where("id", id).delete();
